...
void inference(float* prob, int* ans, float* aa_mass, float* dp, float* dpMass ,int* lock,float premass,int length,float grid_size, float tol2) {
    ...
    int h = threadIdx.x;
    if(w == 0 || h == 0)return;
    float maxMass = aa_mass[AA_num - 13] * h;
...
def knapDecode(prob, preMass, tol):
    ...
    # AAmasses = [0.0, 71.037114, 115.026943, 129.042593, 147.068414, 57.021464, 137.058912, 113.084064, 128.094963, 113.084064, 131.040485, 114.042927, 237.147727, 
    #             97.052764, 128.058578, 156.101111, 87.032028, 101.047679, 150.953633, 99.068414, 186.079313, 163.063329,
    #             160.030649, 147.0354, 42.010565]
