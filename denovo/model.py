...
from components.transformers import PeptideDecoder, SpectrumEncoder
...
# # new residues by Liupu
aa2mas = {'A': 71.037114, 'D': 115.026943, 'E': 129.042593, 'F': 147.068414, 'G':  57.021464, 'H': 137.058912, 'I': 113.084064, 'K': 128.094963, 'L': 113.084064, 'M': 131.040485, 'N': 114.042927, 'O': 237.147727, 'P': 97.052764, 'Q': 128.058578, 'R': 156.101111,  'S': 87.032028, 'T': 101.047679, "U": 150.953633,  'V': 99.068414, 'W': 186.079313, 'Y': 163.063329, 'C+57.021': 160.030649, 'M+15.995': 147.0354, '+42.011': 42.010565, "_": 0}
    ...
    **kwargs : Dict
    ...
    def __init__(
        ...
        **kwargs: Dict,
        ...
        # self.tb_summarywriter = SummaryWriter("/linjiaying/XuanjiNovo/logs/eval_fs")
    ...
    def training_step(
        ...
        *args,
        ...
        if (mode == "train"):
            ...
            self.tb_summarywriter.add_scalar("train/aa_precision", aa_precision, self.steps - 1)
            self.tb_summarywriter.add_scalar("train/aa_recall", aa_recall, self.steps - 1)
            self.tb_summarywriter.add_scalar("train/pep_recall", pep_recall, self.steps - 1)
            self.tb_summarywriter.add_scalar("train/mask_ratio", mask_ratio, self.steps - 1)
        ...
        if (mode == "valid" and self.n_beams==0):
            ...
            self.tb_summarywriter.add_scalar("valid/aa_precision", aa_precision, self.val_steps - 1)
            self.tb_summarywriter.add_scalar("valid/aa_recall", aa_recall, self.val_steps - 1)
            self.tb_summarywriter.add_scalar("valid/pep_recall", pep_recall, self.val_steps - 1)
            self.tb_summarywriter.add_scalar("valid/mr", mr, self.steps - 1)
        ...
        if (mode == "test" and self.n_beams==0):
            ...
            self.tb_summarywriter.add_scalar("test/aa_precision", aa_precision, self.test_steps - 1)
            self.tb_summarywriter.add_scalar("test/aa_recall", aa_recall, self.test_steps - 1)
            self.tb_summarywriter.add_scalar("test/pep_recall", pep_recall, self.test_steps - 1)  
            self.tb_summarywriter.add_scalar("test/mr", mr, self.steps - 1)          
        ...
        if (mode == "train"):
            ...
            self.tb_summarywriter.add_scalar("train/loss", loss.detach(), self.steps - 1)
            lr = self.trainer.optimizers[0].param_groups[0]['lr']
            self.tb_summarywriter.add_scalar("optim/lr", lr, self.steps - 1)
            self.tb_summarywriter.add_scalar("optim/epoch", self.trainer.current_epoch, self.steps - 1)
            self.steps += 1
    ...
    def validation_step(self, batch, batch_idx=None, dataloader_idx=None) -> torch.Tensor:
        ...
        if self.tb_summarywriter is not None:
            if key == "valid":
                self.tb_summarywriter.add_scalar("valid/loss", loss.detach(), self.val_steps - 1)
                self.val_steps += 1
            else:
                self.tb_summarywriter.add_scalar("test/loss", loss.detach(), self.test_steps - 1)
                self.test_steps += 1 
