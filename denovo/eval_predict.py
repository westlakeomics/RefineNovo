...
class PTModule(pl.LightningModule):
    ...
    def test_step(
        ...
        try:
            ...
            for step in range(3):
                if step == 0:
                    sequences = self.model.decoder.get_mask_idx() * torch.ones(len(peptides), self.model.decoder.max_length).long()
                else:
    ...
    def on_test_epoch_end(self) -> None:
        ...
        output_filepath = os.path.join(self.config['out_path'], f"test_predict_pred_guomics.csv")
