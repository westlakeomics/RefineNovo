...
class PeptideDecoder(_PeptideTransformer):
    ...
    def __init__(
        ...
        print("Before max_length:", max_length)
    ...
    def forward(self, sequences, precursors, memory, memory_key_padding_mask, prev = None):
        ...
        # Feed through model:
        tgt = precursors.repeat(1, self.max_length, 1)    # b_z, max_len, dim
        
        
        if prev is None:
            prev = self.get_mask_idx() * torch.ones(tgt.shape[0], self.max_length).long().to(self.device)
